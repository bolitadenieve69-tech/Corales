...
function JoinContent() {
    ...
    const handleJoin = async (e: React.FormEvent) => {
        e.preventDefault()
        if (!selectedRole || !code || !email || !password || !fullName) return
        setIsLoading(true)
        setError("")
        try {
            ...
            setError(err.message || "Error al registrarse. Comprueba los datos o el límite del coro.")
            setIsLoading(false)
        }
    }
        ...
        <div className="flex-1 flex items-center justify-center relative z-10 w-full max-w-xl mx-auto">
            <AnimatePresence mode="popLayout">
                ...
                {step === "code" && (
                    ...
                    >
                        <div className="text-center mb-10">
                            ...
                            <p className="text-neutral-300">Introduce el código de invitación que te proporcionó el director.</p>
                        </div>
                        ...
                        <form onSubmit={(e) => handleVerifyCode(e)} className="space-y-6">
                            <div className="relative group">
                                <input
                                    ...
                                    className="w-full bg-[#0a0f1c] border border-white/10 rounded-2xl px-6 py-5 text-center text-3xl font-mono text-white tracking-[0.2em] placeholder:text-slate-700 focus:outline-none focus-visible:outline-2 focus-visible:outline-accent-500 focus-visible:outline-offset-2 focus:ring-2 focus:ring-indigo-500/50 transition-all uppercase"
                                    placeholder="CÓDIGO"
                                />
                ...
                {step === "role" && (
                    ...
                    >
                        <div className="text-center mb-10">
                            <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full bg-indigo-500/10 border border-indigo-500/20 text-primary-300 text-xs font-bold uppercase tracking-widest mb-4">
                                <CheckCircle2 size={14} /> Código Aceptado
                            </div>
                            <h2 className="text-4xl font-bold text-white mb-2">{choirName}</h2>
                            <p className="text-neutral-300">¿Qué cuerda cantas en este coro?</p>
                        </div>
                        ...
                        <button
                            ...
                            className="w-full mt-6 text-neutral-600 text-sm font-medium hover:text-white transition-colors"
                        >
                ...
                {step === "details" && (
                    ...
                    >
                        <div className="text-center mb-10">
                            <h2 className="text-4xl font-bold text-white mb-2">Crea tu cuenta</h2>
                            <p className="text-neutral-300">Introduce tus datos para acceder a <b>{choirName}</b></p>
                        </div>
                        ...
                        <form onSubmit={handleJoin} className="space-y-4">
                            <div className="space-y-1.5">
                                ...
                                <input
                                    ...
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus-visible:outline-2 focus-visible:outline-accent-500 focus-visible:outline-offset-2 focus:ring-2 focus:ring-accent-500 focus-visible:outline-2 focus-visible:outline-accent-500"
                            ...
                            <button
                                type="submit"
                                disabled={isLoading}
                                className="w-full mt-6 bg-gradient-to-r from-blue-600 to-indigo-600 text-white font-semibold rounded-2xl px-6 py-4 flex items-center justify-center gap-2 hover:shadow-lg transition-all"
                            >
                                {isLoading ? <Loader2 className="animate-spin" size={20} /> : "Finalizar Registro"}
                            </button>

                            <button
                                type="button"
                                onClick={() => setStep("role")}
                                className="w-full mt-4 text-neutral-600 text-sm font-medium hover:text-white transition-colors"
                            >
                                Volver a selección de cuerda
                            </button>
                        </form>
                    </motion.div>
                )}
...
export default function JoinPage() {
    return (
        <div className="min-h-screen bg-primary-900 flex flex-col p-4 md:p-8 font-sans">
            <div className="fixed inset-0 overflow-hidden pointer-events-none">
                <div className="absolute top-[-20%] right-[-10%] w-[50%] h-[50%] bg-purple-600/20 rounded-full blur-[120px]" />
                <div className="absolute bottom-[-20%] left-[-10%] w-[50%] h-[50%] bg-primary-500/20 rounded-full blur-[120px]" />
            </div>

            <Suspense fallback={<div className="flex-1 flex items-center justify-center w-full max-w-xl mx-auto z-10 relative"><Loader2 className="w-8 h-8 animate-spin text-white" /></div>}>
                <JoinContent />
            </Suspense>
        </div>
    )
