...
import { useAcademyDashboard, useLessons } from '@/hooks/useAcademy';
import type { Lesson } from '@/hooks/useAcademy';
...
export default function AcademyPage() {
    ...
    return (
        <QueryBoundary isLoading={isDashboardLoading || isLessonsLoading} error={null} onRetry={() => refetchDashboard()}>
            {dashboardData && (() => {
                // Completed lessons: the first N lessons by order are completed
                const sortedAll = [...dashboardData.lessons].sort((a: any, b: any) => (a.order ?? 0) - (b.order ?? 0));
                const completedIds = new Set(sortedAll.slice(0, dashboardData.completed_lessons).map((l: any) => l.id));
                const levelCompletedCount = filteredLessons.filter((l: any) => completedIds.has(l.id)).length;
