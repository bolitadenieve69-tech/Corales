...
export default function ProjectsPage() {
    ...
    if (loading) {
        return <div className="p-12 text-center text-slate-500">Cargando proyectos...</div>;
    }
                ...
                {projects.map((project, i) => (
                    <Link href={`/projects/${project.id}`} key={project.id}>
                        <motion.div
                            ...
                            className="group bg-[#0a0a1a] border border-white/10 rounded-2xl p-6 hover:border-blue-500/50 hover:bg-[#0f0f24] transition-all cursor-pointer relative overflow-hidden"
                        >
                            ...
                            <div className="flex justify-between items-start mb-4">
                                <h3 className="text-xl font-semibold text-white group-hover:text-blue-400 transition-colors line-clamp-2 leading-tight">
                                    {project.name}
                                </h3>
                                {project.isPublished ? (
                            ...
                            <div className="space-y-3 mt-6">
                                <div className="flex items-center gap-3 text-slate-400">
                                    ...
                                    <span className="text-sm">{new Date(project.date).toLocaleDateString('es-ES', { year: 'numeric', month: 'long', day: 'numeric' })}</span>
                                </div>
                                <div className="flex items-center gap-3 text-slate-400">
                                    <Music4 size={16} className="text-slate-500" />
                                    <span className="text-sm">{project.repertoireCount} obras asignadas</span>
                                </div>
                            </div>
