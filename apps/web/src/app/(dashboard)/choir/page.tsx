...
export default function ChoirPage() {
    ...
    // Check role access
    useEffect(() => {
        if (!authLoading && user && user.role === 'CORALISTA') {
        ...
        <div className="space-y-8 animate-in fade-in duration-500">
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
                ...
                <div className="lg:col-span-1 space-y-6">
                    <div className="bg-primary-800 border border-white/10 rounded-2xl p-8 flex flex-col items-center">
                        <div className="relative group cursor-pointer">
                            <div className="w-32 h-32 rounded-2xl bg-gradient-to-br from-blue-500/20 to-indigo-500/20 border-2 border-dashed border-white/20 flex items-center justify-center group-hover:border-primary-500/50 transition-all overflow-hidden text-primary-500/50 text-4xl font-bold">
                                {choirName.charAt(0)}
                            </div>
                        ...
                        <p className="text-neutral-600 text-sm mt-1">Perfil de Coro</p>
                    </div>

                    <div className="bg-primary-800 border border-white/10 rounded-2xl p-6 space-y-4">
                        <div className="flex items-center justify-between text-sm">
                            <span className="text-neutral-300">Cupo de Miembros</span>
                            <span className="font-medium text-primary-300">{invites.filter((i: any) => i.uses_count > 0).length + 1} / {choir.max_users || 50}</span>
                        </div>
                        <div className="flex items-center justify-between text-sm">
                            <span className="text-neutral-300">Invitaciones Activas</span>
                ...
                <div className="lg:col-span-2 space-y-6">
                    <div className="bg-primary-800 border border-white/10 rounded-2xl p-8">
                        <h3 className="text-xl font-semibold mb-6">Información General</h3>
                        <form onSubmit={handleSave} className="space-y-6">
                            <div className="space-y-2">
                                <label htmlFor="choir-name" className="text-sm font-medium text-neutral-300">Nombre del Coro</label>
                                <input
                                    id="choir-name"
                                    type="text"
                                    value={choirName}
                                    onChange={(e) => setChoirName(e.target.value)}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus-visible:outline-2 focus-visible:outline-accent-500 focus-visible:outline-offset-2 focus:ring-2 focus:ring-accent-500 focus-visible:outline-2 focus-visible:outline-accent-500 transition-all font-medium"
                                    placeholder="Nombre de tu coro"
                                />
                            </div>
                            <div className="space-y-2">
                                <label htmlFor="description" className="text-sm font-medium text-neutral-300">Descripción / Biografía</label>
                                <textarea
                                    id="description"
                                    rows={4}
                                    value={description}
                                    onChange={(e) => setDescription(e.target.value)}
                                    className="w-full bg-white/5 border border-white/10 rounded-xl px-4 py-3 text-white focus:outline-none focus-visible:outline-2 focus-visible:outline-accent-500 focus-visible:outline-offset-2 focus:ring-2 focus:ring-accent-500 focus-visible:outline-2 focus-visible:outline-accent-500 resize-none"
                            ...
                            <button
                                type="submit"
                                disabled={saving}
                                className="flex items-center gap-2 px-6 py-3 bg-primary-500 hover:bg-primary-500 text-white rounded-xl font-medium transition-all shadow-lg shadow-glow-primary disabled:opacity-50"
                            >
                    ...
                    <div className="bg-primary-800 border border-white/10 rounded-2xl p-8">
                        <div className="flex items-center justify-between mb-8">
                            <div>
                                ...
                                <p className="text-sm text-neutral-300 mt-1">Crea enlaces para que tus coralistas se unan automáticamente.</p>
                            </div>
                            <button
                        ...
                        {loadingInvites ? (
                                ...
                                {invites.map((invite) => {
                                    ...
                                    return (
                                        <div key={invite.id} className={`p-4 rounded-xl border ${isUsed ? 'bg-white/5 border-white/5 opacity-60' : 'bg-indigo-500/5 border-indigo-500/20'} flex items-center justify-between group`}>
                                            <div className="flex items-center gap-4">
                                                <div className={`p-2 rounded-lg ${isUsed ? 'bg-slate-500/10 text-neutral-600' : 'bg-primary-500/20 text-primary-300'}`}>
                                                    <Users size={20} />
                                                </div>
                                                <div>
                                            ...
                                            <div className="flex items-center gap-2">
                                                {!isUsed && (
                                                    <button
                                                        ...
                                                        className="p-2 hover:bg-indigo-500/10 text-primary-300 rounded-lg transition-colors"
                                                        title="Copiar enlace"
                                                    >
                                                        <Save size={18} />
                                                    </button>
                            ...
                            <div className="text-center py-12 border-2 border-dashed border-white/5 rounded-2xl">
                                ...
                                <p className="text-neutral-600 text-sm">No has creado ninguna invitación aún.</p>
                            </div>
                        )}
                    </div>
                </div>
